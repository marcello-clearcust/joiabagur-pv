...
Permite importar stock desde Excel (.xlsx o .xls) con columnas SKU y Quantity. El punto de venta se selecciona antes de subir el archivo. Si el producto ya tiene stock, el valor se suma (positivo) o resta (negativo); si no, se crea el registro en `Inventory` (asignación implícita del producto al POS, solo para cantidades positivas). La importación es todo o nada: si alguna fila haría que el stock quede negativo, se rechaza el archivo completo.

**Formato Excel:** SKU (obligatorio, debe existir en catálogo global), Quantity (obligatorio, número entero positivo o negativo; el stock resultante no puede ser negativo)
...
### Escenario 1: Importar stock para productos existentes (cantidad positiva)
...
**Y** se crea movimiento en `InventoryMovement` tipo "Import" con QuantityChange positivo

### Escenario 1b: Restar stock con cantidad negativa
**Dado que** un producto tiene stock 10 en un punto de venta  
**Cuando** importo Excel con Quantity -3 para ese producto  
**Entonces** el stock se actualiza a 7 (10 - 3)  
**Y** se crea movimiento en `InventoryMovement` tipo "Import" con QuantityChange = -3

### Escenario 1c: Rechazar importación cuando el resultado sería negativo
**Dado que** un producto tiene stock 2 en un punto de venta  
**Cuando** importo Excel con Quantity -5 para ese producto  
**Entonces** el sistema rechaza el archivo completo  
**Y** devuelve error: "Fila X: El stock resultante para '{SKU}' sería negativo (actual: 2, cambio: -5, resultado: -3)."  
**Y** ningún registro de Inventory o InventoryMovement es modificado
